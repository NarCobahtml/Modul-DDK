<?php
include "cek.php";
include "koneksi.php";

$result = $conn->query("SELECT * FROM news ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dashboard | Portal Berita</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body { background: #f3f6fb; min-height: 100vh; }
        .sidebar { background: #111827; min-height: 100vh; }
        .news-thumb { width: 96px; height: 64px; object-fit: cover; border-radius: 8px; }
    </style>
</head>
<body>
<div class="container-fluid">
    <div class="row">
        <aside class="col-md-3 col-lg-2 sidebar text-white p-4">
            <h4 class="fw-bold">Portal Berita</h4>
            <p class="small text-white-50 mb-4">Role: <?= htmlspecialchars($_SESSION['level']) ?></p>
            <div class="d-grid gap-2">
                <a class="btn btn-outline-light" href="dashboard.php">Dashboard</a>
                <a class="btn btn-danger mt-3" href="logout.php">Logout</a>
            </div>
        </aside>
        <main class="col-md-9 col-lg-10 p-4 p-md-5">
            <div class="bg-white rounded-4 shadow-sm p-4 p-md-5 mb-4">
                <h1 class="fw-bold mb-2">Selamat datang, <?= htmlspecialchars($_SESSION['nama_lengkap']) ?></h1>
                <p class="lead mb-0">Anda login sebagai <strong><?= htmlspecialchars($_SESSION['level']) ?></strong>.</p>
            </div>

            <?php if (isset($_GET['pesan']) && $_GET['pesan'] === 'akses'): ?>
                <div class="alert alert-warning">Akses ditolak. Level user hanya dapat melihat daftar dan detail berita.</div>
            <?php endif; ?>

            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-body p-4">
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                        <div>
                            <h3 class="fw-bold mb-1">Data Berita</h3>
                            <p class="text-muted mb-0">Daftar berita dari tabel <code>news</code>.</p>
                        </div>
                        <?php if ($_SESSION['level'] === 'admin'): ?>
                            <a href="insert.php" class="btn btn-primary">+ Tambah Berita</a>
                        <?php endif; ?>
                    </div>

                    <div class="table-responsive">
                        <table class="table table-bordered align-middle">
                            <thead class="table-dark">
                            <tr>
                                <th style="width: 70px;">No</th>
                                <th>Judul</th>
                                <th>Konten</th>
                                <th>Author</th>
                                <th>Gambar</th>
                                <th>Tanggal</th>
                                <th style="width: 210px;">Aksi</th>
                            </tr>
                            </thead>
                            <tbody>
                            <?php $no = 1; while ($row = $result->fetch_assoc()): ?>
                                <tr>
                                    <td><?= $no++ ?></td>
                                    <td class="fw-semibold"><?= htmlspecialchars($row['title']) ?></td>
                                    <td><?= htmlspecialchars(substr($row['content'], 0, 100)) ?>...</td>
                                    <td><?= htmlspecialchars($row['author']) ?></td>
                                    <td>
                                        <?php if (!empty($row['image'])): ?>
                                            <img class="news-thumb" src="upload/<?= htmlspecialchars($row['image']) ?>" alt="<?= htmlspecialchars($row['title']) ?>">
                                        <?php else: ?>
                                            <span class="text-muted">Tidak ada</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?= htmlspecialchars($row['date']) ?></td>
                                    <td>
                                        <a class="btn btn-sm btn-info text-white" href="detail.php?id=<?= $row['id'] ?>">Detail</a>
                                        <?php if ($_SESSION['level'] === 'admin'): ?>
                                            <a class="btn btn-sm btn-warning" href="update.php?id=<?= $row['id'] ?>">Edit</a>
                                            <a class="btn btn-sm btn-danger" href="delete.php?id=<?= $row['id'] ?>" onclick="return confirm('Hapus berita ini?')">Delete</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </main>
    </div>
</div>
</body>
</html>
